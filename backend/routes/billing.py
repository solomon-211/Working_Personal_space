...
# route to create an invoice from a consultation visit with free-text line items
@billing_bp.route('/invoices/from-visit', methods=['POST'])
@role_required('admin', 'receptionist')
def create_invoice_from_visit():
    """
    Creates an invoice from a visit using free-text items (e.g. drugs, ad-hoc services).
    Items are matched to the services catalogue by name; unrecognised names are
    auto-created in the catalogue so that FK integrity on invoice_items is maintained.
    Expected body:
    {
        "patient_id": 1,
        "visit_id": 5,                  (optional — stored as appointment_id on the invoice)
        "discount": 500,                (optional, default 0)
        "items": [
            { "service_name": "Paracetamol — 500mg (5 days)", "quantity": 1, "unit_price": 200.00 }
        ]
    }
    """
    data = request.get_json()
    ...
    try:
        ...
        for item in data['items']:
            ...
            # Look up the service by name; create it if it doesn't exist yet
            cursor.execute(
                "SELECT service_id FROM services WHERE service_name = %s", (name,)
            )
            svc = cursor.fetchone()
            if svc:
                ...
                ins_cursor.execute(
                    "INSERT INTO services (service_name, unit_price, category) VALUES (%s, %s, 'Other')",
                    (name, price)
                )
                service_id = ins_cursor.lastrowid
                cache_invalidate('services:all')

            subtotal = qty * price
            total   += subtotal
            line_items.append((service_id, qty, price, subtotal))

        if not line_items:
            conn.close()
            return jsonify({'error': 'No valid items with a non-zero price were provided'}), 400
        ...
        invoice_id = ins_cursor.lastrowid

        ins_cursor.executemany("""
            INSERT INTO invoice_items (invoice_id, service_id, quantity, unit_price, subtotal)
            VALUES (%s, %s, %s, %s, %s)
        """, [(invoice_id, sid, qty, price, sub) for sid, qty, price, sub in line_items])
