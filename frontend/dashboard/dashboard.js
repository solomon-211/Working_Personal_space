    ...
    <table style="width:100%">
      ...
      <tbody>
        ${appointmentsList.map(a => `
          <tr>
            <td>${formatTime(a.appointment_datetime)}</td>
...
function renderRecentPatients(patients) {
  ...
  element.innerHTML = recentPatients.map(p => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);gap:12px;">
      <div>
        ...
        <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">${p.clinic_number} · Registered ${formatDate(p.registered_at)}</div>
      </div>
      <a href="/patients/profile.html?id=${p.patient_id}" class="btn btn-outline btn-sm">View</a>
    </div>
  `).join('');
}
